...
const BuatLaporanPage = () => {
  ...
  const handleExitCancel = () => {
    ...
    setStep(1);
    setFormData(prev => ({ ...prev, judul: '', kategori: '', urgensi: 'Sedang', deskripsi: '', tanggal: '', riwayat: 'Tidak', alamat: '', kelurahan: '', kecamatan: '', jenis_laporan: 'PUBLIK', nama_pelapor_offline: '', foto: [] }));
    setPreviews([]);
    setAgree(false);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (res.data.success) {
        setSuccess(true);
        localStorage.removeItem('draft_laporan');
        setTimeout(() => navigate('/laporan'), 3000);
      }
    } catch (err) {
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                       {categories.length === 0 ? (
                         <div className="col-span-3 text-center py-8 text-gray-400">
                           ...
                           <p className="text-sm font-bold">{lang === 'ID' ? 'Memuat kategori...' : 'Loading categories...'}</p>
                         </div>
                       ) : categories.map((cat) => (
                         <button
                           key={cat.id}
                         ...
                         >
                            ...
                            <span className="text-xs font-bold">{cat.nama}</span>
                         </button>
                       ))}
                    </div>
                 </div>
                 ...
                 <div className="bg-gray-50/50 dark:bg-gray-800/30 rounded-[32px] p-8 space-y-6">
                    ...
                    <div className="grid gap-4">
                       ...
                       <div className="flex justify-between items-center">
                          ...
                          <span className="px-3 py-1 bg-green-50 dark:bg-green-900/20 text-[#2d5a1e] text-[10px] font-bold rounded-full">{categories.find(c => c.id === formData.kategoriId)?.nama || '-'}</span>
                       </div>
                       <div className="flex justify-between items-center">
