...
import { 
  ...
  BarChart3, Activity, Settings, HelpCircle,
  Search, Bell, LogOut, ChevronRight, Menu, X,
  Moon, Sun, Languages, Shield
} from 'lucide-react';
...
const AdminLayout = ({ children }) => {
  ...
  const menuItems = [
    { icon: LayoutDashboard, label: 'Dashboard', to: '/admin/dashboard', labelKey: 'nav_dashboard' },
    { icon: FileText, label: 'Kelola Laporan', to: '/admin/laporan', labelKey: 'side_manage_report' },
    { icon: Layers, label: 'Kelola Kategori', to: '/admin/kategori', labelKey: 'side_manage_category' },
    { icon: BarChart3, label: 'Statistik Wilayah', to: '/admin/statistik', labelKey: 'nav_stats' },
  ];
      ...
      <aside className={`fixed inset-y-0 left-0 z-50 bg-white dark:bg-gray-900 border-r border-gray-200 dark:border-gray-800 transition-all duration-300 lg:translate-x-0 ${isSidebarOpen ? 'w-72 translate-x-0' : 'w-[88px] lg:translate-x-0 -translate-x-full'}`}>
        <div className={`h-full flex flex-col ${isSidebarOpen ? 'p-6' : 'p-4'}`}>
            ...
            <div className={`flex items-center gap-3 transition-all duration-300 overflow-hidden ${isSidebarOpen ? 'opacity-100 mb-6' : 'opacity-0 w-0 h-0 mb-0'}`}>
              ...
              <div className="whitespace-nowrap">
                ...
                <div className="flex items-center gap-1.5">
                   ...
                   <p className="text-[10px] font-black text-[#2d5a1e] dark:text-green-400 uppercase tracking-[0.2em]">{translations[lang]?.side_admin_panel}</p>
                </div>
              </div>
            </div>
          ...
          <div className={`mt-auto pt-6 border-t border-gray-100 dark:border-gray-800 space-y-2 ${!isSidebarOpen ? 'flex flex-col items-center' : ''}`}>
             ...
             <Link to="/admin/pengaturan" className={`w-full flex items-center gap-4 px-4 py-4 rounded-2xl text-sm font-bold text-gray-600 dark:text-gray-300 bg-gray-50 dark:bg-gray-800 hover:bg-gray-100 dark:hover:bg-gray-700 transition-all ${!isSidebarOpen ? 'justify-center' : ''}`} title={!isSidebarOpen ? translations[lang]?.side_settings : ''}>
                <Settings className="w-5 h-5 shrink-0" />
                <span className={`transition-all duration-300 overflow-hidden whitespace-nowrap ${isSidebarOpen ? 'opacity-100 w-auto' : 'opacity-0 w-0'}`}>
                  {translations[lang]?.side_settings}
                </span>
             </Link>
             <Link to="/admin/bantuan" className={`w-full flex items-center gap-4 px-4 py-4 rounded-2xl text-sm font-bold text-white bg-[#2d5a1e] hover:bg-[#1e3f14] transition-all shadow-lg shadow-green-900/20 active:scale-95 ${!isSidebarOpen ? 'justify-center' : ''}`} title={!isSidebarOpen ? translations[lang]?.side_help : ''}>
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-6">
               <div className="flex items-center gap-2 border-r border-gray-100 dark:border-gray-800 pr-6">
                  ...
                  <Link to="/admin/notifikasi" className="w-10 h-10 rounded-xl flex items-center justify-center text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors relative">
               ...
               <div className="flex items-center gap-4">
                  <Link to="/admin/pengaturan" className="flex items-center gap-4 group cursor-pointer">
                    <div className="text-right hidden sm:block">
                       <p className="text-[15px] font-black text-gray-900 dark:text-white group-hover:text-[#2d5a1e] transition-colors leading-none mb-1">{user?.nama || 'Admin'}</p>
                       <p className="text-[11px] text-gray-400 font-bold">{user?.email || 'admin@laporinaja.id'}</p>
                    </div>
                    <div className="w-11 h-11 rounded-2xl overflow-hidden border-2 border-white dark:border-gray-800 shadow-sm group-hover:shadow-md group-hover:border-[#2d5a1e] transition-all bg-green-50 dark:bg-green-900/20 flex items-center justify-center text-sm font-bold text-[#2d5a1e] dark:text-green-400">
                       {user?.foto_profil ? (
                         <img src={`http://localhost:5000${user.foto_profil}`} alt="Admin" className="w-full h-full object-cover" />
                       ) : (
                         user?.nama?.[0]?.toUpperCase() || 'AD'
                       )}
                    </div>
                  </Link>
               </div>
